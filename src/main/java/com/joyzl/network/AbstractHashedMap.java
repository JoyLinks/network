...
public class AbstractHashedMap<K, V> extends AbstractMap<K, V> {
	...
	public boolean equals(final Object obj) {
		...
		try {
			while (it.hasNext()) {
				...
				if (entry.getValue() == null) {
					if (map.get(entry.getKey()) != null || map.containsKey(entry.getKey()) == false) {
						return false;
					}
				} else {
					if (entry.getValue().equals(map.get(entry.getValue())) == false) {
						return false;
					}
				}
			}
	...
	public String toString() {
		...
		while (hasNext) {
			final Map.Entry<K, V> entry = it.next();
			buf.append(entry.getKey() == this ? "(this Map)" : entry.getKey()).append('=').append(entry.getValue() == this ? "(this Map)" : entry.getValue());
