 ...
 */
public class V1ClientHandler implements ChainHandler {

	private final ChainHandler handler;
	private final TLSParameters parameters;
	private final TLSShare share;

	private final V1CipherSuiter cipher = new V1CipherSuiter();
	private final V0SecretCache secret = new V0SecretCache();
	private final Signaturer signaturer = new Signaturer();

	private byte[] clientVerify, serverVerify;
	private byte[] sessionId;

	public V1ClientHandler(ChainHandler handler, TLSParameters parameters, TLSShare share) {
		this.parameters = parameters;
		this.handler = handler;
		this.share = share;
	}

	public V1ClientHandler(ChainHandler handler, TLSParameters parameters) {
		this(handler, parameters, new TLSShare());
	}

	public V1ClientHandler(ChainHandler handler) {
	...
	public DataBuffer encode(ChainChannel chain, Object message) throws Exception {
		if (message instanceof Record record) {
			...
			if (record.contentType() == Record.HANDSHAKE) {
				if (record instanceof Handshakes handshakes) {
					...
					for (int i = 0; i < handshakes.size(); i++) {
						...
						if (handshakes.get(i).msgType() == Handshake.FINISHED) {
							RecordCoder.encodeV1(ChangeCipherSpec.INSTANCE, buffer);
						}
						if (cipher.encryptReady()) {
							RecordCoder.encodeCiphertext(cipher, record, data, buffer);
						} else {
							RecordCoder.encodePlaintextV1(record, data, buffer);
						}
					}
					data.release();
					return buffer;
				...
				final DataBuffer buffer = DataBuffer.instance();
				RecordCoder.encodeV1((ChangeCipherSpec) record, buffer);
				return buffer;
			} else if (record.contentType() == Record.APPLICATION_DATA) {
				final DataBuffer buffer = DataBuffer.instance();
				RecordCoder.encodeV1((ApplicationData) record, buffer);
				return buffer;
	...
	private Handshake decode(DataBuffer buffer) throws Exception {
		...
		if (type == Handshake.SERVER_HELLO) {
			...
			return HandshakeCoder.decodeV0(buffer);
		} else if (type == Handshake.FINISHED) {
			// 服务端完成消息校验码
			final byte[] local = secret.serverFinished();
			secret.hash(buffer, length);

			final Finished finished = (Finished) HandshakeCoder.decodeV0(buffer);
	...
	private void encode(Handshake handshake, DataBuffer buffer) throws Exception {
		if (handshake.msgType() == Handshake.CLIENT_HELLO) {
			HandshakeCoder.encodeV0(handshake, buffer);
			...
			final CertificateVerifyV0 verify = (CertificateVerifyV0) handshake;
			// 生成客户端证书消息签名
			verify.setSignedMD5(secret.hashMD5());
			verify.setSignedSHA(secret.hashSHA());
	...
	private Record handshake(Handshake record) throws Exception {
		if (record.msgType() == Handshake.SERVER_HELLO) {
			...
			final CertificateRequestV0 request = (CertificateRequestV0) record;
			final CertificateV0 certificate = new CertificateV0();
			// final LocalCache local =
			// SessionCertificates.filters(request.getNames(),request.getTypes());
			// if (local != null) {
			// final CertificateVerifyV0 certificateVerify = new
			// CertificateVerifyV0();
			//
