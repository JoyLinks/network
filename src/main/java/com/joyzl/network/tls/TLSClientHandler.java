...
public class TLSClientHandler extends RecordHandler {
	...
	protected void encode(Handshake handshake, DataBuffer buffer) throws Exception {
		...
		if (handshake.msgType() == Handshake.CLIENT_HELLO) {
			...
			if (extension != null && extension instanceof PreSharedKeys) {
				// 0-RTT BinderKey
				final PreSharedKeys offeredPsks = (PreSharedKeys) extension;
		...
		if (message.msgType() == Handshake.SERVER_HELLO) {
			...
			if (serverHello.isHelloRetryRequest()) {
				...
				if (serverHello.getVersion() == TLS.V12) {
					final SelectedVersion selectedVersion = serverHello.getExtension(Extension.SUPPORTED_VERSIONS);
					if (selectedVersion != null) {
						if (selectedVersion.get() == TLS.V13) {
							// OK 1.3
						} else {
			...
			if (certificate.size() > 0) {
				...
				signaturer.setEntries(certificate.get());
			...
			newSessionTicket.setResumption(cipher.resumption(newSessionTicket.getNonce()));
