 ...
 */
public class V3ServerHandler extends TLS implements ChainHandler {

	private final ChainHandler handler;
	private final TLSParameters parameters;

	public V3ServerHandler(ChainHandler handler, TLSParameters parameters) {
	...
	private Handshake decode(TLSContext context, DataBuffer buffer) throws Exception {
		...
		if (type == Handshake.CLIENT_HELLO) {
			...
			buffer.mark();
			final Handshake handshake = HandshakeCoder.decodeV3(buffer);
			// 待协商后执行消息哈希
		...
		if (message instanceof Record record) {
			...
			if (record.contentType() == Record.HANDSHAKE) {
				if (record instanceof Handshakes handshakes) {
					...
					final DataBuffer buffer = DataBuffer.instance();
					if (context.secret.isHandshaked()) {
						RecordCoder.encodeCiphertext(context.cipher, record, data, buffer);
					} else {
						RecordCoder.encodePlaintextV2(record, data, buffer);
					}
	...
	private void encode(TLSContext context, Handshake handshake, DataBuffer buffer) throws Exception {
		if (handshake.msgType() == Handshake.FINISHED) {
			...
			// 这是最先加密的握手消息
			// 导出密钥并重置握手加密和解密套件
			// 因早期数据还须解密，此时不应重置解密套件
			context.secret.serverHandshakeTrafficSecret();
			context.secret.clientHandshakeTrafficSecret();
			context.cipher.encryptReset(//
				context.secret.serverHandshakeWriteKey(context.cipher.type()), //
				context.secret.serverHandshakeWriteIV(context.cipher.type()));
		} else if (handshake.msgType() == Handshake.CERTIFICATE_VERIFY) {
			// 生成服务端证书消息签名
			final CertificateVerifyV3 verify = (CertificateVerifyV3) handshake;
			verify.setSignature(context.signaturer.singServer(context.secret.hash()));
	...
	public void received(ChainChannel chain, Object message) throws Exception {
		final TLSContext context = chain.getContext(TLSContext.class);
		if (message == null) {
			if (context.secret.isApplication()) {
				handler.received(chain, message);
			} else {
				chain.close();
			}
		...
		if (message instanceof Record) {
			...
			if (message instanceof Alert) {
				...
				if (handshakes.last().msgType() == Handshake.FINISHED) {
					if (context.secret.isHandshaked()) {
						// 重置握手数据解密套件
						context.cipher.decryptReset(//
							context.secret.clientHandshakeWriteKey(context.cipher.type()), //
							context.secret.clientHandshakeWriteIV(context.cipher.type()));
						// 导出应用数据加密密钥并重置加密套件
						// 应用数据解密密钥导出，等待客户端完成消息后重置解密套件
						context.secret.serverApplicationTrafficSecret();
						context.secret.clientApplicationTrafficSecret();
						context.cipher.encryptReset(//
							context.secret.serverApplicationWriteKey(context.cipher.type()), //
							context.secret.serverApplicationWriteIV(context.cipher.type()));
						// context.secret.exporterMaster();
				...
				if (update.get() == KeyUpdate.UPDATE_NOT_REQUESTED) {
					...
					context.cipher.encryptReset(//
						...
						context.secret.serverApplicationWriteIV(context.cipher.type()));
					context.cipher.decryptReset(//
						context.secret.clientApplicationWriteKey(context.cipher.type()), //
						context.secret.clientApplicationWriteIV(context.cipher.type()));
				}
			}
		} else {
	...
	private Record handshake(TLSContext context, Handshake record) throws Exception {
		if (record.msgType() == Handshake.CLIENT_HELLO) {
			...
			// selected_versions
			server.addExtension(new SelectedVersion(TLS.V13));
			...
			if (psks != null) {
				...
				if (psks.size() > 0) {
					...
					if (ticket != null) {
						...
						context.cipher.initialize(ticket.getSuite());
						context.secret.reset(ticket.getResumption());
						context.secret.hash(context.data, length);
						// 丢弃已计算的数据
						context.data.skipBytes(length);
						length = context.length -= length;
						if (identity.check(context.secret.resumptionBinderKey())) {
							// 3 0-RTT
							// 补充消息哈希
							context.secret.hash(context.data, length);
							context.data.skipBytes(length);
							...
							if (earlyData != null) {
								// 导出早期流量密钥
								context.secret.clientEarlyTrafficSecret();
								context.cipher.decryptReset(//
									context.secret.clientEarlyWriteKey(context.cipher.type()), //
									context.secret.clientEarlyWriteIV(context.cipher.type()));
								// context.secret.earlyExporterMaster();
							} else {
								// 未有早期数据
							}
					...
					if (shares.size() > 0) {
						...
						for (i = 0; i < shares.size(); i++) {
							...
							if (groups.check(entry.getGroup())) {
								if (NamedGroup.check(entry.getGroup(), parameters.namedGroups())) {
									break;
								} else {
									continue;
								}
						...
						if (entry != null && i < shares.size()) {
							...
							// 设置对端共享密钥（公钥）
							context.secret.sharedKey(context.key.sharedKey(entry.getKeyExchange()));
							// TranscriptHash:client_hello
							if (context.length > 0) {
								context.secret.hash(context.data, context.length);
						...
						final short group = groups.match(parameters.namedGroups());
						if (group > 0) {
							// Hello Retry Request
							// 请求客户端提供对应算法的密钥后重试
							server.makeHelloRetryRequest();
							server.addExtension(new KeyShareHelloRetryRequest(group));
							server.addExtension(new Cookie(context.secret.hash()));
							context.secret.retry();
							return server;
						} else {
							// 没有匹配的密钥算法
							return new Alert(Alert.HANDSHAKE_FAILURE);
			...
			if (alpn != null) {
				if (alpn.select(parameters.alpns())) {
		...
		if (record.msgType() == Handshake.KEY_UPDATE) {
			...
			if (update.get() == KeyUpdate.UPDATE_REQUESTED) {
				...
				context.secret.nextApplicationTrafficSecret();
				return update;
		...
		if (record.msgType() == Handshake.FINISHED) {
			...
			if (finished.validate()) {
				...
				context.cipher.decryptReset(//
					context.secret.clientApplicationWriteKey(context.cipher.type()), //
					context.secret.clientApplicationWriteIV(context.cipher.type()));
				context.secret.resumptionMasterSecret();
				if (context.mode >= 0) {
					// NewSessionTicket
					final Handshakes records = new Handshakes();
					...
					ticket1.setResumption(context.secret.resumptionSecret(ticket1.getNonce()));
					ticket2.setResumption(context.secret.resumptionSecret(ticket2.getNonce()));
					ticket1.setSuite(context.cipher.type().code());
					ticket2.setSuite(context.cipher.type().code());
	...
	private class TLSContext extends TLSSlaveContext {
		...
		final V3CipherSuiter cipher;
		final V3SecretCache secret;
		final V3KeyExchange key;
		byte mode;

		public TLSContext() {
			signaturer = new Signaturer();
			cipher = new V3CipherSuiter();
			secret = new V3SecretCache();
