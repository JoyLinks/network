 ...
 */
public class V1ServerHandler extends ServerHandler implements ChainHandler {

	public V1ServerHandler(ChainHandler handler, TLSParameters parameters) {
	...
	private Handshake decode(TLSContext context, DataBuffer buffer) throws Exception {
		...
		if (type == Handshake.CLIENT_HELLO) {
			...
			buffer.mark();
			final Handshake handshake = HandshakeCoder.decodeV0(buffer);
			// 待协商后执行消息哈希
	...
	private class TLSContext extends TLSSlaveContext {
		...
		final V1CipherSuiter cipher;
		final V0SecretCache secret;

		public TLSContext() {
			signaturer = new Signaturer();
			cipher = new V1CipherSuiter();
