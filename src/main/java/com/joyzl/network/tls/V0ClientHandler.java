 ...
 */
public class V0ClientHandler implements ChainHandler {

	private final ChainHandler handler;
	private final TLSParameters parameters;
	private final TLSShare share;

	private final V0CipherSuiter cipher = new V0CipherSuiter();
	...
	public V0ClientHandler(ChainHandler handler, TLSParameters parameters, TLSShare share) {
		this.parameters = parameters;
		this.handler = handler;
		this.share = share;
	}

	public V0ClientHandler(ChainHandler handler, TLSParameters parameters) {
		this(handler, parameters, new TLSShare());
	}

	public V0ClientHandler(ChainHandler handler) {
				...
				if (record instanceof Handshakes handshakes) {
					...
					for (int i = 0; i < handshakes.size(); i++) {
						...
						if (handshakes.get(i).msgType() == Handshake.FINISHED) {
							RecordCoder.encodeV0(ChangeCipherSpec.INSTANCE, buffer);
						}
						if (cipher.encryptReady()) {
							RecordCoder.encodeCiphertext(cipher, record, data, buffer);
						} else {
							RecordCoder.encodePlaintextV0(record, data, buffer);
						}
					}
					data.release();
					return buffer;
